...
def mock_openai_chat_client():
    with patch("mem0.proxy.main.OpenAI") as mock_openai:
        mock_client = Mock()
        mock_openai.return_value = mock_client
        yield mock_client
...
def test_completions_create(mock_memory_client, mock_openai_chat_client):
    completions = Completions(mock_memory_client)

    messages = [{"role": "user", "content": "Hello, how are you?"}]
    mock_memory_client.search.return_value = [{"memory": "Some relevant memory"}]
    mock_response = Mock()
    mock_openai_chat_client.chat.completions.create.return_value = mock_response
    ...
    mock_openai_chat_client.chat.completions.create.assert_called_once()
    call_args = mock_openai_chat_client.chat.completions.create.call_args[1]
...
def test_completions_create_with_system_message(mock_memory_client, mock_openai_chat_client):
    completions = Completions(mock_memory_client)
    ...
    mock_memory_client.search.return_value = [{"memory": "Some relevant memory"}]
    mock_openai_chat_client.chat.completions.create.return_value = Mock()
